...
sumTo n
  | n <= 0 = 0  -- Base case: empty sum
  | otherwise = n + sumTo (n - 1)  -- Recursive case: add n to sum of 1...n-1
...
power base exponent
  | exponent <= 0 = if base == 0 then error "0^0 is undefined" else 1  -- Handle base case and error
