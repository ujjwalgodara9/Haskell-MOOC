...
countAndLog p xs = Logger (map show filtered) (length filtered)
    where filtered = filter p xs
...
rob from to = balance from +> \amt -> withdrawOp from amt +> \_ -> depositOp to amt
...
paren c = do
    count <- get
    case c of 
        '(' -> if count == -1 then put (-1) else put (count + 1)
...
count x = do
    state <- get
    let newState = case lookup x state of
                     Just n  -> (x, n + 1) : filter ((/= x) . fst) state
                     Nothing -> (x, 1) : state
    put newState
