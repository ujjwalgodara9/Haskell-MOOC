...
prettyPrint size queens = concat [printRow r | r <- [1..size]]
  where
    printRow r = [if (r,c) `elem` queens then 'Q' else '.' | c <- [1..size]] ++ "\n"
...
danger _ [] = False
danger candidate (q:qs) = 
    sameRow candidate q || sameCol candidate q || sameDiag candidate q || sameAntidiag candidate q || danger candidate qs
  where
    sameRow (i, _) (k, _) = i == k
    sameCol (_, j) (_, l) = j == l
...
prettyPrint2 :: Int -> Stack -> String
prettyPrint2 size queens = unlines [ [square (i, j) | j <- [1..size]] | i <- [1..size] ]
  where
    square coord
      | elem coord queens = 'Q'
      | danger coord queens = '#'
...
isSafe :: Size -> Stack -> (Int, Int) -> Bool
isSafe n s (x, y) = not $ any (\(x', y') -> x == x' || y == y' || abs (x - x') == abs (y - y')) s

fixFirst :: Size -> Stack -> Maybe Stack
fixFirst n [] = Nothing
fixFirst n ((x,y):qs) 
  | y > n = Nothing
  | isSafe n qs (x,y) = Just ((x,y):qs)
  | otherwise = fixFirst n ((x,y+1):qs)
...
finish n s
  | length s == n = s
  | otherwise = case step n s of
      s' -> if length s' > length s then finish n s' else s
