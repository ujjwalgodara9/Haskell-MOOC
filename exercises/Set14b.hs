...
import Database.SQLite.Simple (Connection, Query, execute, query, Only(..))
...
balance db account = do
  amounts <- query db balanceQuery (Only account) :: IO [Only Int]
  return $ sum [x | Only x <- amounts]
...
data Command = Deposit T.Text Int | Balance T.Text deriving (Show, Eq)

parseInt :: T.Text -> Maybe Int
parseInt = readMaybe . T.unpack

parseCommand :: [T.Text] -> Maybe Command
parseCommand (action:account:xs)
  | action == T.pack "deposit", not (null xs), Just amount <- parseInt (head xs) = Just (Deposit account amount)
  | action == T.pack "balance" = Just (Balance account)
...
perform _ Nothing = return $ T.pack "Error: Invalid Command"
