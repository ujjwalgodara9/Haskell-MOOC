...
isSorted list = property $ list == sort list
...
genList = do
  length <- choose (3, 5)
  list <- vectorOf length $ choose (0, 10)
  return $ sort list
...
instance Arbitrary Arg where
  arbitrary = oneof [ numberGen, variableGen ]
    where
      numberGen = Number <$> choose (0, 10)
      variableGen = Variable <$> elements ['a', 'b', 'c', 'x', 'y', 'z']

instance Arbitrary Expression where
  arbitrary = oneof [ plusGen, minusGen ]
    where
      plusGen = Plus <$> arbitrary <*> arbitrary
      minusGen = Minus <$> arbitrary <*> arbitrary
