...
allSums :: [Int] -> [Int]
allSums = foldr (\x acc -> acc ++ map (+x) acc) [0]
...
instance Functor SL where
  fmap f (SL g) = SL $ \s ->
    let (a, newState, log) = g(s)
    in (f a, newState, log)

instance Applicative SL where
  pure x = SL $ \s -> (x, s, [])
  (SL f) <*> (SL g) = SL $ \s ->
    let (fab, newState, log1) = f s
        (a, finalState, log2) = g newState
    in (fab a, finalState, log1 ++ log2)

instance Monad SL where
  return x = SL $ \s -> (x, s, [])
  (SL g) >>= f = SL $ \s ->
    let (a, newState, log1) = g s
        (SL h) = f a
        (b, finalState, log2) = h newState
    in (b, finalState, log1 ++ log2)
