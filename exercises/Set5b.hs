...
treeMax Empty = 0
treeMax (Node value leftTree rightTree) = max value (max (treeMax leftTree) (treeMax rightTree))
...
allValues condition (Node value leftTree rightTree) =
    condition value && allValues condition leftTree && allValues condition rightTree
...
mapTree f (Node value leftTree rightTree) =
    Node (f value) (mapTree f leftTree) (mapTree f rightTree)
...
cull _ Empty = Empty
cull val (Node value leftTree rightTree)
    | val == value = Empty
    | otherwise = Node value (cull val leftTree) (cull val rightTree)
...
isOrdered (Node value leftTree rightTree) =
    allValues (< value) leftTree &&
    allValues (> value) rightTree &&
    isOrdered leftTree &&
    isOrdered rightTree
