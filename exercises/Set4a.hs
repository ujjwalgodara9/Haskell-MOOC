...
longest :: (Ord a) => [[a]] -> [a]
longest = foldr1 (\x y -> if length x > length y || (length x == length y && head x <= head y) then x else y)
...
transfer from to amount bank
  ...
  | currentFromAmount < amount = bank
  | otherwise = Map.insert to (currentToAmount + amount) . Map.insert from (currentFromAmount - amount) $ bank
  where
    currentFromAmount = Map.findWithDefault 0 from bank
    currentToAmount = Map.findWithDefault 0 to bank
