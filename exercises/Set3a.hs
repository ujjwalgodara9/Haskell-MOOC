...
-- Define the infix operator +|+ with a precedence level.
infixr 5 +|+

-- The operator takes two lists and returns a list.
(+|+) :: [a] -> [a] -> [a]
(x:_) +|+ (y:_) = [x, y]
[] +|+ (y:_)   = [y]
(x:_) +|+ []   = [x]
...
interpreter = snd . foldl processCommand ((0,0), [])
  where
    processCommand ((x,y), prints) command = case command of
      "up"     -> ((x, y+1), prints)
      "down"   -> ((x, y-1), prints)
      "left"   -> ((x-1, y), prints)
      "right"  -> ((x+1, y), prints)
      "printX" -> ((x, y), prints ++ [show x])
      "printY" -> ((x, y), prints ++ [show y])
      _        -> ((x, y), prints) 
